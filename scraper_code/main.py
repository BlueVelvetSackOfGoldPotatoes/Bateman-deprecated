...
def main():
    ...
    for publisher_key, links in publishers_links.items():
        journal_name = publisher_key.replace("_links", "").lower()
        ...
        for link in links:
            ...
            try:
                if "themedcollections" in link:
                    ...
                    _, temp_count, vpn_index = scrape_page_articles_rsc(
                        link, OUTPUT_FOLDER, CSV_PATH, journal_name, vpn_index
                    )
                elif "journalissues" in link:
                    # Scraper for Specific Issues
                    _, temp_count, vpn_index = scrape_issue_page_rsc(
                        link, OUTPUT_FOLDER, CSV_PATH, journal_name, vpn_index
                    )
                else:
