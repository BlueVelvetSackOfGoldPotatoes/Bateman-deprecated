...
def extract_metadata(doi: str, csv_path: str) -> Optional[str]:
    """Fetch metadata from multiple sources using DOI and append to CSV."""
    try:
        # Try CrossRef API first
        metadata = fetch_metadata_via_doi(doi)
        
        # If CrossRef doesn't have the data, try the DOI Content Negotiation API
        if not metadata or metadata.get('Title') == 'N/A':
            metadata = fetch_metadata_via_doi_org(doi)
        
        # If still no metadata, set default values
        if not metadata or metadata.get('Title') == 'N/A':
...
def fetch_metadata_via_doi_org(doi: str) -> Optional[dict]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            metadata = {
                ...
                "Journal": item.get('container-title', 'N/A'),
                "Authors": ', '.join([
                    f"{author.get('given', '')} {author.get('family', '')}".strip()
                    for author in item.get('author', [])
                ]),
                "Keywords": ', '.join(item.get('keyword', [])),
                "Institute of Origin": item.get('publisher', 'N/A'),
                "Funding": ', '.join([f.get('name', '') for f in item.get('funder', [])])
            }
            return metadata
        else:
            logger.error(f"Failed to fetch metadata via doi.org for DOI {doi}: Status Code {response.status_code}")
            return None
    except Exception as e:
        logger.error(f"Exception occurred while fetching metadata via doi.org for DOI {doi}: {e}")
        traceback.print_exc()
        return None
...
def fetch_metadata_via_doi(doi: str) -> Optional[dict]:
    ...
    try:
        # Ensure the DOI is complete by adding the prefix if missing
        if not doi.startswith('10.'):
            doi = '10.1039/' + doi
